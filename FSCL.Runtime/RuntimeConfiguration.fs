...
type RuntimeConfiguration(defSteps, sources: SourceConfiguration list) =
    ...
    member internal this.ToXml() =
        let el = new XElement(XName.Get(this.GetType().Name),
                    new XAttribute(XName.Get("LoadDefault"), this.LoadDefault),
    ...
    static member internal FromXml(doc: XDocument, srcRoot: string) =
        ...
        let loadDefault = if(doc.Root.Attribute(XName.Get("LoadDefault")) <> null) then
                            bool.Parse(doc.Root.Attribute(XName.Get("LoadDefault")).Value)
                          else
                             false
    ...
    member internal this.MakeExplicit() =
        ...
        RuntimeConfiguration(this.LoadDefault, List.ofSeq sources)
